...
export default function Permission() {
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useGetRole(page, limit, searchQuery);

  useEffect(() => {
    refetch();
  }, [limit, page, searchQuery, refetch, roles]);
        ...
        cell: function ActionCell({ row }) {
          return (
            <div className="flex gap-2">
              ...
              row.original?.name?.toLowerCase() != "superadmin" &&
              (
                <Eye
                  size={26}
  ...
  if (isError) {
    return <ErrorComponent message="Error fetching data" onRetry={refetch} />;
